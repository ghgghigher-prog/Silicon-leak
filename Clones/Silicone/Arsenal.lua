...
local HumanTab = Window:CreateTab("Humanoid", "user")
...
HumanTab:CreateParagraph({
    ...
    Content = "You are using Silicon for Arsenal."
})

HumanTab:CreateDivider()

HumanTab:CreateSection("Humanoid")
...
HumanTab:CreateToggle({
    ...
    Callback = function(Value)
        ...
        if not char or not char:FindFirstChild("HumanoidRootPart") then
            flyEnabled = false
            Silicon:Notify({Title = "Fly", Content = "Character not found!"})
            return
        end
        ...
        if Value then
            ...
            Silicon:Notify({Title = "Fly", Content = "Fly has been enabled."})
            ...
            flyConnection = game:GetService("RunService").RenderStepped:Connect(function()
                ...
                if moveDir.Magnitude > 0 then
                    ...
                    root.AssemblyLinearVelocity = worldDir * flySpeed
                else
                    root.AssemblyLinearVelocity = Vector3.new()  
                end
            end)
            ...
            Silicon:Notify({Title = "Fly", Content = "Fly has been disabled."})
        end
    end,
})
...
HumanTab:CreateButton({
    ...
    Callback = function()
        Silicon:Notify({
            Title = "Infinite Jump",
            Content = "Inf Jump has initiated successfully."
        })
    ...
    Callback = function(value)
        ...
        if not value then
            ...
            if ESPEnabled then
                Silicon:Notify({
                    Title = "TeamCheck",
                    Content = "TeamCheck has been enabled."
                })
                for _, player in Players:GetPlayers() do
                    if player ~= LocalPlayer and player.Character then
                        CreateESP(player)
            ...
            Silicon:Notify({
                Title = "TeamCheck",
                Content = "TeamCheck has been disabled."
            })
            for plr, _ in pairs(ESP) do
                if IsFriendly(plr) then ClearESP(plr) end
            end
        end
    end
})
...
AimTab:CreateParagraph({Title = "Welcome to Silicon!", Content = "You are using Silicon for Arsenal."})
...
local function GetClosest()
    ...
    for _, plr in Players:GetPlayers() do
        ...
        if AimbotTeamCheck and plr.Team == LocalPlayer.Team and plr.Team ~= nil then continue end
        if not plr.Character or not plr.Character:FindFirstChild(TargetPart) then continue end
...
AimTab:CreateToggle({ 
    ...
    Callback = function(v)
        Silicon:Notify({
            Title = "TeamCheck",
            Content = Value and "TeamCheck has been enabled." or "TeamCheck has been disabled."
        }) 
        AimbotTeamCheck = v 
    end 
})
